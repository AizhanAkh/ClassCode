...
cdef class Class:
    ...
    def get_Vk_dmeff(self):
        """
        Return a tuple of z, k, Vrms, Vflow, Vfull for dmeff.
        """
        if (not self.pt.has_velocity_transfers):
            print('Cannot return Vrms without velocity transfers.')
            return {},{},{},{},{}

        cdef int index_md
        index_md = self.sp.index_md_scalars

        cdef np.ndarray[DTYPE_t, ndim=1] z    = np.zeros(self.pt.tau_size,'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] k    = np.zeros(self.pt.k_size[index_md],'float64')
        cdef np.ndarray[DTYPE_t, ndim=2] Vrms = np.zeros((self.pt.tau_size,self.pt.k_size[index_md]),'float64')
        cdef np.ndarray[DTYPE_t, ndim=2] Vflow= np.zeros((self.pt.tau_size,self.pt.k_size[index_md]),'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] Vfull= np.zeros(self.pt.tau_size,'float64')
        pvecback = <double*> calloc(self.ba.bg_size,sizeof(double))
        ...
        for index_k in xrange(self.pt.k_size[index_md]):
            k[index_k] = self.pt.k[index_md][index_k]

        for index_tau in xrange(self.pt.tau_size):
            tau = self.pt.tau_sampling[index_tau]
            if background_at_tau(&self.ba,tau,self.ba.long_info,self.ba.inter_normal,&last_index,pvecback)==_FAILURE_:
                raise CosmoSevereError(self.ba.error_message)

            z[index_tau] = 1./pvecback[self.ba.index_bg_a] - 1.
            Vfull[index_tau] = self.sp.Vrms_dmeff[index_tau]

            for index_k in xrange(self.pt.k_size[index_md]):
                Vrms[index_tau,index_k] = self.sp.Vrms_k_dmeff[index_tau * self.pt.k_size[index_md] + index_k]
                Vflow[index_tau,index_k] = self.sp.Vflow_k_dmeff[index_tau * self.pt.k_size[index_md] + index_k]

        free(pvecback)
        return z[::-1],k,Vrms[::-1,:],Vflow[::-1,:],Vfull[::-1]

    def get_Vk_dmeff_dynamic(self):
        """
        Return a tuple of z, k, Vrms, Vflow, Vfull for dmeff.
        """
        if (not self.pt.dmeff_Vrms_dynamic) and (not self.pt.dmeff_Vfull_dynamic):
            print('Dynamic Vrms was not asked for.')
            return {},{},{},{},{}

        cdef int index_md
        index_md = self.sp.index_md_scalars

        cdef np.ndarray[DTYPE_t, ndim=1] z    = np.zeros(self.ba.bt_size,'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] k    = np.zeros(self.pt.k_size[index_md],'float64')
        cdef np.ndarray[DTYPE_t, ndim=2] Vrms = np.zeros((self.ba.bt_size,self.pt.k_size[index_md]),'float64')
        cdef np.ndarray[DTYPE_t, ndim=2] Vflow= np.zeros((self.ba.bt_size,self.pt.k_size[index_md]),'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] Vfull= np.zeros(self.ba.bt_size,'float64')
        pvecback = <double*> calloc(self.ba.bg_size,sizeof(double))
        ...
        for index_tau in xrange(self.ba.bt_size):
            tau = self.ba.tau_table[index_tau]
            if background_at_tau(&self.ba,tau,self.ba.long_info,self.ba.inter_normal,&last_index,pvecback)==_FAILURE_:
                raise CosmoSevereError(self.ba.error_message)

            z[index_tau] = 1./pvecback[self.ba.index_bg_a] - 1.
            Vfull[index_tau] = pvecback[self.ba.index_bg_Vrms]
            for index_k in xrange(self.pt.k_size[index_md]):
                Vrms[index_tau,index_k]  =  self.pt.Vrms_k_dmeff_dynamic[index_tau * self.pt.k_size[index_md] + index_k]
                Vflow[index_tau,index_k] = self.pt.Vflow_k_dmeff_dynamic[index_tau * self.pt.k_size[index_md] + index_k]
